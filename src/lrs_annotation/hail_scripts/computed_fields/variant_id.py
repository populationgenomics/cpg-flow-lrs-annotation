...
def get_expr_for_contig_number(locus: hl.expr.LocusExpression) -> hl.expr.Int32Expression:
...
def get_expr_for_variant_type(table: hl.Table) -> hl.str:
    return hl.bind(
        lambda ref_len, alt_len: (
            hl.case().when(ref_len > alt_len, 'D').when(ref_len < alt_len, 'I').when(ref_len > 1, 'M').default('S')
        ),
