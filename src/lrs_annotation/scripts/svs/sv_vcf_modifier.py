...
def read_sex_mapping_file(sex_mapping_file_path: str) -> dict[str, int]:
    ...
    """
    sex_mapping = {}
    with open(sex_mapping_file_path) as f:
        lines = f.readlines()
        for line in lines:
...
def translate_var_and_sex_to_cn(contig: str, var_type: str, genotype: str, sex: int) -> int | str:
    ...
    if sex == 1 and contig in [CHRX, CHRY]:
        copy_number = 1
    elif sex == 2 and contig == CHRY:  # noqa: PLR2004
        copy_number = 0
    else:
        copy_number = 2
...
def modify_sv_vcf(file_in: str, file_out: str, fa: str, sex_mapping_file: str):  # noqa: PLR0915
    ...
    with gzip.open(file_in, 'rt') as f, gzip.open(file_out, 'wt') as f_out:
        ...
        for index, line in enumerate(f):
            ...
            # e.g. AN_Orig=61;END=56855888;SVTYPE=DUP  # noqa: ERA001
            ...
            for i, sample_id in enumerate(sample_ids, start=9):
                ...
                gt_string = dict(zip(l_split[8].split(':'), l_split[i].split(':'), strict=False))['GT']
